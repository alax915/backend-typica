...
app.use(cors({
    origin: 'https://ale-site.netlify.app' // Put your real Netlify URL here
}));
...
app.post('/login', async(req, res) => {
    ...
    try {
        ...
        if (!response.ok) {
            // This triggers if password is wrong or user doesn't exist
            return res.status(401).json({ error: "Invalid phone number or password" });
        }

        console.log(`✅ Secure Login successful for UID: ${data.localId}`);
...
app.post('/api/bank/save', async (req, res) => {
    const { uid, bankName, accountNumber, accountHolder } = req.body;
    ...
    try {
        ...
        await userRef.update({
            bankAccount: {
                ...
                isVerified: false, // Default to false for admin review
...
app.get('/api/user/profile-summary/:uid', async (req, res) => {
    try {
        ...
        } catch (e) { console.log("⚠️ Transactions skip:", e.message); }

        // 5. IMPROVED EARNINGS LOGIC
        // We calculate for each timeframe to ensure product income shows even without check-ins
        const timeframes = [
            { key: 'today', start: startOfToday, end: new Date(now.getTime() + 86400000) },
            { key: 'yesterday', start: startOfYesterday, end: startOfToday }
        ];

        // A. Calculate Product Income (Automatic)
            ...
            userData.purchasedProducts.forEach(prod => {
                if (prod.status === 'active') {
                    ...
                    if (pDate <= startOfToday) {
                        earnings.today += income;
                    }
                    // Yesterday's Income (if they owned it then)
                    if (pDate <= startOfYesterday) {
                        earnings.yesterday += income;
                    }
                    // Week Income (Simple estimation: income * days owned in last 7)
                    const daysOwned = Math.min(7, Math.max(1, Math.floor((now - pDate) / 86400000) + 1));
                    earnings.week += (income * daysOwned);
                    
                    // Month Income
                    const daysThisMonth = Math.max(1, Math.floor((now - Math.max(pDate, startOfMonth)) / 86400000) + 1);
                    earnings.month += (income * daysThisMonth);
                }
            });
        }

        // B. Add Check-in Bonuses (Manual)
        if (userData.dailyCheckins && Array.isArray(userData.dailyCheckins)) {
            userData.dailyCheckins.forEach(dateStr => {
                const d = new Date(dateStr);
                const bonus = 1; // 1 ETB per check-in
...
app.get('/api/user/profile-summary/:uid', async (req, res) => {
    try {
        ...
        // 2. Initialize Defaults (Prevents "NaN" errors)
        let earnings = { today: 0, yesterday: 0, week: 0, month: 0 };
        let totals = { revenue: 0, withdrawal: 0, recentRecharge: 0 };
        ...
        try {
            ...
            if (!transSnap.empty) {
                transSnap.forEach(doc => {
                    ...
                    if (t.type === 'recharge') {
                        totals.revenue += amt;
                        totals.recentRecharge = amt; // Stores the last one found
                    } else if (t.type === 'withdraw') {
                        totals.withdrawal += amt;
                    }
                });
            ...
            console.log("⚠️ Transactions collection issue:", e.message); 
        }

        // 5. Safe Check-in & Product Income Calculation
        if (userData.dailyCheckins && Array.isArray(userData.dailyCheckins)) {
            userData.dailyCheckins.forEach(dateStr => {
                const d = new Date(dateStr);
                let dailyTotal = 1; // Base 1 ETB per check-in

                if (userData.purchasedProducts && Array.isArray(userData.purchasedProducts)) {
                    userData.purchasedProducts.forEach(prod => {
                        if (new Date(prod.purchaseDate) <= d && prod.status === 'active') {
                            dailyTotal += (Number(prod.dailyIncome) || 0);
                        }
                    });
                }

                if (d >= startOfToday) {
                    earnings.today += dailyTotal;
                } else if (d >= startOfYesterday && d < startOfToday) {
                    earnings.yesterday += dailyTotal;
                }
                
                if (d > new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000)) earnings.week += dailyTotal;
                if (d > new Date(now.getFullYear(), now.getMonth(), 1)) earnings.month += dailyTotal;
            });
        }

        // 6. FINAL CLEAN RESPONSE (The critical fix is here)
        res.json({
            // THIS LINE IS REQUIRED: The frontend needs the real phone to query products
            phoneNumber: userData.phoneNumber || userData.phone || "Unknown",
            
            // Masked phone for display only
            maskedPhone: (userData.phoneNumber || "Unknown").replace(/(\+\d{3})(\d{3})\d+(\d{2})/, "$1$2****$3"),
...
app.get('/api/user/withdrawals/:uid', async (req, res) => {
    ...
    try {
        // Securely query Firestore using the Admin SDK
        ...
        snapshot.forEach(doc => {
            ...
            transactions.push({
                id: doc.id,
                ...data,
                // Ensure timestamp is sent in a readable format
...
app.get('/api/user/withdrawals/:uid', async (req, res) => {
    ...
    try {
        ...
        console.log(`📊 Result: Found ${snapshot.size} documents in Firestore`);

        const transactions = [];
        snapshot.forEach(doc => {
            const data = doc.data();
            // Convert Firestore timestamp to JS Date for the frontend
...
// --- 🏦 UPDATED: SAVE BANK & PHONE DETAILS ---
app.post('/api/bank/save', async (req, res) => {
...
app.post('/api/user/update-password', async (req, res) => {
    ...
    try {
        // Use Admin SDK to update the Auth record
        await auth.updateUser(uid, {
            password: newPassword
        });

        // Also update the plaintext version in Firestore if you are tracking it there
        // (Note: Security hardening would recommend NOT storing plaintext passwords)
        await db.collection('users').doc(uid).update({
            password: newPassword 
        });
